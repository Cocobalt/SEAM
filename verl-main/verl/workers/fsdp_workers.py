...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        # build device mesh for Tensor Model Parallel and Ulysses Sequence Parallel
        if self._is_actor or self._is_ref:
            self.device_mesh = create_device_mesh(world_size=world_size, fsdp_size=self.config.actor.fsdp_config.fsdp_size)
            self.ulysses_sequence_parallel_size = self.config.actor.get("ulysses_sequence_parallel_size", 1)
        elif self._is_rollout:
            self.device_mesh = create_device_mesh(world_size=world_size, fsdp_size=self.config.rollout.fsdp_config.fsdp_size)
            self.ulysses_sequence_parallel_size = self.config.rollout.get("ulysses_sequence_parallel_size", 1)
        else:
    ...
    def _build_model_optimizer(
        ...
        fsdp_strategy = self.config.actor.strategy if self.config.get("actor", None) is not None else "fsdp"
    ...
    def _switch_to_grm_template(self, data: DataProto):
        ...
        # ---- 输入拆分 ----
        prompts = data.batch["prompts"]              # [bs, P]
        responses = data.batch["responses"]          # [bs, R]
        full_attention_mask = data.batch["attention_mask"]  # [bs, P+R]

        bs = prompts.shape[0]
        P = prompts.shape[1]
        R = responses.shape[1]

        prompt_attention_mask = full_attention_mask[:, :P]
        response_attention_mask = full_attention_mask[:, P:P+R]

        rm_input_ids_list = []
        rm_attention_mask_list = []

        self._src_tokenizer = AutoTokenizer.from_pretrained(
            self.config.rollout.src_tokenizer_path, use_fast=True, trust_remote_code=True
        )

        assert self.config.rollout.template_file is not None, "template_file is not set"
        with open(self.config.rollout.template_file, "r", encoding="utf-8") as f:
            grm_template = f.read()
        ...
        for i in range(bs):
            ...
            p_ids = prompts[i][prompt_attention_mask[i].bool()].tolist()
            r_ids = responses[i][:valid_len].tolist()

            prompt_text = self._src_tokenizer.decode(p_ids, skip_special_tokens=False)
            response_text = self._src_tokenizer.decode(r_ids, skip_special_tokens=False)

            ground_truth = data.non_tensor_batch["reward_model"][i]["ground_truth"]
            grm_content = grm_template.format(ground_truth=ground_truth)

            messages = [{"role": "user", "content": f"{prompt_text}{response_text}\n{grm_content}"}]
            tpl = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
            prompt_str = tpl + "<think>\n"

            grm_tokens = self.tokenizer(prompt_str, add_special_tokens=False)["input_ids"]
            grm_tensor = torch.tensor(grm_tokens, dtype=prompts.dtype, device=prompts.device)
            grm_attn = torch.ones_like(grm_tensor, dtype=full_attention_mask.dtype, device=full_attention_mask.device)

            # 路径A只送入重分词后的 grm_tensor
            rm_input_ids_list.append(grm_tensor)
            rm_attention_mask_list.append(grm_attn)



        target_len = int(full_attention_mask.shape[1]) + 512
        pad_token_id = self.tokenizer.pad_token_id

        rm_input_ids = []
        rm_attention_mask = []

        for seq_ids, seq_mask in zip(rm_input_ids_list, rm_attention_mask_list):
            L = int(seq_ids.shape[0])

            if L > target_len:
                seq_ids = seq_ids[-target_len:]
                seq_mask = seq_mask[-target_len:]
                L = target_len

            pad_len = target_len - L
            padded_ids = torch.full((target_len,), pad_token_id, dtype=prompts.dtype, device=prompts.device)
            padded_mask = torch.zeros((target_len,), dtype=full_attention_mask.dtype, device=full_attention_mask.device)

            padded_ids[pad_len:] = seq_ids
            padded_mask[pad_len:] = seq_mask

            rm_input_ids.append(padded_ids)
            rm_attention_mask.append(padded_mask)

        rm_input_ids = torch.stack(rm_input_ids)          # [bs, target_len]
        rm_attention_mask = torch.stack(rm_attention_mask)  # [bs, target_len]
    ...
    def generate_sequences_as_grm(self, data: DataProto, **kwargs):
        ...
        with self.rollout_sharding_manager:
            ...
            if self.config.rollout.name == "sglang_async":
                raise ValueError("AsyncSGLangRollout is not supported for GRM currently")
            else:
                output = self.rollout.generate_sequences(prompts=prompts, **self.config.rollout)
